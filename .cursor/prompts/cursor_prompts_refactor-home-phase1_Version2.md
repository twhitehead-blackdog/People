...
Pasos:
1) Crear `home-api.service.ts` en `src/app/dashboard/services/` para llamadas HTTP/Supabase (usando `ApiUrlService`).
2) Crear `home-utils/` con funciones puras (mapeos, agregaciones, orden, métricas, formato).
