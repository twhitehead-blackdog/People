-- ============================================
-- EXPORTAR ESTRUCTURA COMPLETA (VERSIÓN CORREGIDA)
-- ============================================
-- Versión corregida de los queries de Supabase
-- Soluciona el error con funciones de agregación
-- ============================================

-- 1) Tablas y columnas (tipo, default, nulabilidad)
SELECT
  c.table_schema,
  c.table_name,
...
LEFT JOIN pg_catalog.pg_class pc
  ...
  AND pc.relkind IN ('r','p','v','m') -- table, partitioned table, view, materialized view
...
-- 4) Constraints (PRIMARY KEY, UNIQUE, CHECK, EXCLUDE)
SELECT
  n.nspname AS schema_name,
  c.relname AS table_name,
...
-- 5) Funciones almacenadas (stored functions) - CORREGIDO
-- Filtra funciones de agregación que causan error con pg_get_functiondef
SELECT
  n.nspname AS schema_name,
...
WHERE n.nspname NOT IN ('pg_catalog','information_schema')
  -- Opcional: filtrar solo funciones y procedimientos (comentar si quieres ver agregaciones)
