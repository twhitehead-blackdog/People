-- ============================================
-- Migración: Prevención de timelogs duplicados
-- ============================================
-- Modifica la función process_timelog para verificar duplicados
-- antes de insertar. Si existe un timelog del mismo empleado + tipo
-- en los últimos 2 minutos, retorna error en lugar de insertar.
-- ============================================
...
DECLARE
  ...
  v_result JSONB;
BEGIN
  -- 0. Check for duplicate timelog within last 2 minutes for same employee + type
  IF EXISTS (
    SELECT 1 FROM timelogs
    WHERE employee_id = p_employee_id
    AND type = p_type::timelog_type
    AND created_at > NOW() - INTERVAL '2 minutes'
  ) THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Marcación duplicada detectada. Por favor espere 2 minutos antes de marcar nuevamente.',
      'error_code', 'DUPLICATE_TIMELOG'
    );
  END IF;
...
'Procesa una marcación (timelog) consolidando inserción, consultas de horario y cálculos en una sola transacción. Incluye verificación de duplicados (2 min). Retorna JSONB con delay, exitDiff, lunchExceeded y datos del horario.';
