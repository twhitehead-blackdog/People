      ...
      <p-card class="w-full max-w-lg text-center bg-neutral-900 border border-neutral-800">
        <ng-template pTemplate="title">
          <div class="flex flex-col items-center gap-3">
            <div class="relative">
              <div class="w-20 h-20 rounded-full bg-gradient-to-br from-red-500/30 to-red-600/20 flex items-center justify-center shadow-lg shadow-red-500/20 border border-red-500/30 animate-pulse">
                <i class="pi pi-exclamation-triangle text-red-400 text-3xl drop-shadow-lg"></i>
              </div>
              <div class="absolute inset-0 w-20 h-20 rounded-full bg-red-500/10 animate-ping"></div>
            </div>
        ...
        <div class="space-y-4 text-gray-300">
              ...
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-blue-200 text-sm leading-relaxed">
                    Conéctate al WiFi de tu sucursal para poder acceder al modo kiosko. 
                    El sistema detectará automáticamente cuando vuelvas a una IP autorizada y te redirigirá al modo kiosko.
                  </p>
                </div>
              </div>
            </div>
          ...
          @if (currentIP()) {
            <div class="mt-4 p-3 bg-neutral-800/50 rounded-lg border border-neutral-700">
              ...
              <p class="text-sm text-red-400 font-mono font-semibold">{{ currentIP() }}</p>
            </div>
          }
          @if (previousIP()) {
...
export class NoAccessComponent implements OnInit, OnDestroy {
  ...
  description = signal<string>('Tu correo no aparece en nuestra base de empleados. Comunícate con tu supervisor para solicitar acceso.');
    ...
    this.route.queryParams.subscribe(params => {
      ...
      switch (reason) {
        case 'ip_not_allowed':
          ...
          this.description.set('Tu dirección IP actual no está en la lista de IPs permitidas para el modo kiosko.');
        ...
        case 'ip_changed':
          ...
          this.description.set('Se detectó un cambio en tu dirección IP. El modo kiosko solo está disponible desde IPs autorizadas.');
          this.showLogout.set(false);
          this.showWifiInstruction.set(true);
          this.startIPMonitoring();
          break;
        case 'ip_check_failed':
          ...
          this.description.set('Hubo un error al verificar tu dirección IP. Por favor, intenta nuevamente o contacta al administrador.');
          this.showLogout.set(false);
        ...
        case 'no_ips_configured':
          ...
          this.description.set('El modo kiosko no está configurado correctamente. Contacta al administrador del sistema.');
          this.showLogout.set(false);
          break;
        default:
    ...
    this.http.get<Branch[]>(url, {
      ...
      switchMap((branches) => {
        ...
        this.checkInterval = setInterval(() => {
          this.ipMonitor.validateInitialIP(allowedIPs).then((result) => {
