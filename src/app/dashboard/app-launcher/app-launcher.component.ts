...
interface Module {
  ...
  moduleId?: string;
  external?: boolean;
  /** Tailwind color name (emerald, blue, violet, etc.) */
  tw: string;
}
...
// Color map: tw name → { icon hex, bg tailwind, border tailwind, hover-border tailwind, hover-shadow tailwind }
const COLOR_MAP: Record<string, { hex: string; bgCls: string; iconCls: string; hoverBorderCls: string; hoverShadowCls: string }> = {
  ...
  yellow:  { hex: '#fde047', bgCls: 'bg-yellow-500/15',  iconCls: 'text-yellow-400',  hoverBorderCls: 'hover:border-yellow-500/40',  hoverShadowCls: 'hover:shadow-yellow-500/20' },
...
@Component({
  ...
  standalone: true,
  template: `
    <div class="min-h-screen bg-[#0a0a0a] p-4 sm:p-6 md:p-10 space-y-6 md:space-y-8">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-3 md:gap-4 max-w-5xl mx-auto">
        @for (mod of visibleModules(); track mod.id) {
          ...
          <button (click)="open(mod)"
             class="group relative overflow-hidden rounded-xl bg-white/5 backdrop-blur-sm border border-white/10
...
export class AppLauncherComponent {
  ...
  public visibleModules = computed(() =>
    MODULES.filter((m) =>
  ...
  public open(mod: Module): void {
    if (mod.external === false) {
      this.router.navigate(['/' + mod.target]);
    } else if (mod.external === true || mod.target.startsWith('http') || mod.target.startsWith('/')) {
