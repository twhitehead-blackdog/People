...
async function check() {
  ...
  const suspDept = departments.filter((d) =>
    JSON.stringify(d).toLowerCase().includes('pecado')
  );
  console.log('Suspicious Departments:', suspDept);

  console.log('--- Companies ---');
  const companies = await fetchTable('companies');
  const suspComp = companies.filter((c) =>
    JSON.stringify(c).toLowerCase().includes('pecado')
  );
  console.log('Suspicious Companies:', suspComp);

  console.log('--- Employees (sample) ---');
  const employees = await fetchTable('employees');
  const suspEmp = employees.filter((e) =>
    JSON.stringify(e).toLowerCase().includes('pecado')
  );
  console.log('Suspicious Employees:', suspEmp);
}
