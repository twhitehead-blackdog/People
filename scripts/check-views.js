...
async function check() {
  console.log('--- Employees with POS ---');
  const empPos = await fetchTable('employees_with_pos');
  const suspEmpPos = empPos.filter((d) =>
    JSON.stringify(d).toLowerCase().includes('pecado')
  );
  console.log('Suspicious:', suspEmpPos);

  console.log('--- Profiles ---');
  const profiles = await fetchTable('profiles');
  const suspProfiles = profiles.filter((c) =>
    JSON.stringify(c).toLowerCase().includes('pecado')
  );
  console.log('Suspicious Profiles:', suspProfiles);
}
